...
// Make relative URLs absolute against a known base
function resolve(base, rel){
  try { return new URL(rel, base).href; } catch { return rel; }
}

// Extract up to N windows that contain *all* terms (AND)
function windowsAND(text, terms, windowSize=800, maxSnips=12){
  if (!terms.length) return [];
  const low = text.toLowerCase();
  // anchor on most-rare term for efficiency
...
async function load(){
  ...
  const TB = tb.map(x => ({
    _bucket:'Textbooks',
    ...x,
    url_resolved: resolve(URLS.TEXTBOOKS, x.url_txt)   // <- FIX: resolve relative textbook URLs
  }));
  ...
  console.info('[search] loaded catalogs:',
    { textbooks: TB.length, laws: LA.length, rules: RU.length });
...
async function searchBucket(docs, qTerms, box, setCount){
  ...
  for (const d of docs){
    try{
      ...
      const snips = windowsAND(body, qTerms, /*window*/ 900, /*max*/ 18);
      if (snips.length){
