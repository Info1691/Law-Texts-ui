...
let current = {
  ...
  scale: 1.2,         // auto-fit; recalculated on load/resize
  textIndex: {},      // { pageNumber: "plain text of that page" }
  hits: [],           // [{ page, snippet }]
  hitCursor: -1
};
...
async function selectBook(book, liEl){
  ...
  try {
    if (current.isPdf){
      await openPdf(url);
      // Force canvas-based PDF viewer (no iframes)
...
// ---- PDF.js helpers (canvas renderer) ----
...
function autoScaleForWidth(){
  ...
  // ~816px is ~8.5in @ 96dpi, good baseline
...
async function runSearch(){
  ...
  // PDF search (lazy per-page to keep iPad stable)
...
function wireEvents(){
  ...
  els.printBtn.addEventListener("click", () => {
    // Canvas print (no iframe). Let iPad handle page.
    window.print();
  });
  ...
  // Keep fit-to-width rendering on rotation/resize
